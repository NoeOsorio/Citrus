...
class HomeClassVC:UIViewController, UITableViewDelegate, UITableViewDataSource, UICollectionViewDelegate, UICollectionViewDataSource {
                    ...
                    for curso in (cursos?.documents)!{
                        if let titulo = curso.data()["Titulo"] as? String{
                            if let icono = curso.data()["Icono"] as? String{
                                self.cursos.append(["proyecto":titulo,"icono":icono, "materia":materia])
                                self.insertCurso()
                            }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.title.text = cursosContent["proyecto"]! + " : " + cursosContent["materia"]!
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        info.setProyecto(proyecto: selected.contentDicc!["proyecto"]!)
        performSegue(withIdentifier: "proyectito", sender: self)
    }
